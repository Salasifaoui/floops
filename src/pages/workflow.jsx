...
export default function WorkflowPage({
  ...
  onSetActiveTab,
  onCreateBlankTab,
  onCreateTabFromImportedProject,
  onCloseTab,
  onUpdateTab,
}) {
  ...
  const activeFlowDocument = activeTab?.flowDocument ?? flowJson;
  const activeSourceMode = activeTab?.sourceMode ?? "file";
  const initialParsedFlow = useMemo(
    () => parseFlowJson(activeFlowDocument),
    [activeFlowDocument],
  );
  const [sourceFlowDocument, setSourceFlowDocument] =
    useState(activeFlowDocument);
  const [sourceMode, setSourceMode] = useState(activeSourceMode);
  const [nodes, setNodes] = useState(() =>
    buildNodes(initialParsedFlow.tables),
  );
  ...
  const [newColumnForm, setNewColumnForm] = useState(() =>
    getDefaultColumnForm(),
  );
  const [pendingRelationConnection, setPendingRelationConnection] =
    useState(null);
  ...
  const [jsonDraft, setJsonDraft] = useState(() =>
    JSON.stringify(activeFlowDocument, null, 2),
  );
  const [jsonError, setJsonError] = useState("");
  const [jsonStatus, setJsonStatus] = useState("");
  const [projectToOpenId, setProjectToOpenId] = useState("");
  const [isJsonVisible, setIsJsonVisible] = useState(
    Boolean(activeTab?.jsonEditorVisible),
  );
  ...
  const relationDeleteOptions = useMemo(
    () => ["cascade", "setNull", "none"],
    [],
  );
  ...
  useEffect(() => {
    if (!activeTab) return;
    const currentSerialized = JSON.stringify(activeTab.flowDocument ?? {});
    const nextSerialized = JSON.stringify(runtimeFlowDocument);
    if (currentSerialized === nextSerialized) return;
    onUpdateTab(activeTab.id, {
      flowDocument: runtimeFlowDocument,
    });
  }, [activeTab, runtimeFlowDocument, onUpdateTab]);

  const applyFlowDocument = useCallback(
    (nextDocument, mode) => {
  ...
  const onEdgesChange = useCallback((changes) => {
    ...
    setRelations((snapshot) =>
      snapshot.filter(
        (relation) => !removedEdgeIds.has(getRelationEdgeId(relation)),
      ),
    );
  }, []);
  ...
  const onEdgeClick = useCallback(
    (event, edge) => {
      ...
      const relation = relations.find(
        (item) => getRelationEdgeId(item) === edge.id,
      );
      if (!relation) return;
  ...
  const databaseOptions = useMemo(() => {
    const options = new Set(
      nodes.map((node) => String(node.data?.databaseId ?? "unknown")),
    );
    return [...options];
  }, [nodes]);
  ...
  const activeSelectedNodeId =
    selectedNodeId && visibleTableIds.has(selectedNodeId)
      ? selectedNodeId
      : null;
  ...
  const relationPanelSourceNode = useMemo(
    () =>
      pendingRelationConnection
        ? (nodes.find(
            (node) => node.id === pendingRelationConnection.sourceTableId,
          ) ?? null)
        : null,
  ...
  const onSaveRelation = useCallback(
    (event) => {
      ...
      const targetTableId =
        relationForm.relatedTableId || pendingRelationConnection.targetTableId;
      ...
      const onDeleteValue =
        relationForm.onDelete === "none" ? null : relationForm.onDelete;
      ...
      setRelations((snapshot) => {
        ...
        const existingColumnKeys = new Set(
          snapshot
            ...
            .map((relation) =>
              String(relation.sourceColumnKey ?? "").toLowerCase(),
            ),
        );
  ...
  const openEditColumnPanel = useCallback(
    (nodeId, columnIndex) => {
      closeRelationPanel();
  ...
  const onDeleteTable = useCallback(
    (nodeId) => {
      ...
      setRelations((snapshot) =>
        snapshot.filter(
          (relation) =>
            relation.sourceTableId !== nodeId &&
            relation.targetTableId !== nodeId,
        ),
      );
      setSelectedNodeId((snapshot) => (snapshot === nodeId ? null : snapshot));
      setColumnPanelNodeId((snapshot) =>
        snapshot === nodeId ? null : snapshot,
      );
      ...
      setEditingRelationId((snapshot) => {
        ...
        const relation = relations.find(
          (item) => String(item.id) === String(snapshot),
        );
        if (!relation) return null;
        return relation.sourceTableId === nodeId ||
          relation.targetTableId === nodeId
          ? null
          : snapshot;
      });
      ...
      decorateNodes(
        nodes.map((node) => ({
          ...
          data: {
            ...
            onOpenEditColumnPanel: (columnIndex) =>
              openEditColumnPanel(node.id, columnIndex),
  ...
  const onAutoLayout = useCallback(() => {
    ...
    setNodes((snapshot) => {
      const visibleNodes = snapshot.filter((node) =>
        visibleTableIds.has(node.id),
      );
      const hiddenNodes = snapshot.filter(
        (node) => !visibleTableIds.has(node.id),
      );
      const layoutedVisibleNodes = layoutNodesWithDagre(
        visibleNodes,
        visibleEdges,
        "LR",
      );
  ...
  const onResetToSourceJson = useCallback(() => {
    applyFlowDocument(sourceFlowDocument, sourceMode);
    setJsonDraft(JSON.stringify(sourceFlowDocument, null, 2));
    setJsonError("");
    setJsonStatus("Reset to tab source document.");
  }, [applyFlowDocument, sourceFlowDocument, sourceMode]);
    ...
    <div className="app-shell">
        ...
        <div className="app-node-sidebar__json">
          ...
          <div className="app-node-sidebar__json-actions">
            <button
              type="button"
              onClick={() => {
                const nextValue = !isJsonVisible;
                setIsJsonVisible(nextValue);
                if (activeTab) {
                  onUpdateTab(activeTab.id, { jsonEditorVisible: nextValue });
                }
              }}
            >
          ...
          {isJsonVisible ? (
            <>
              ...
              {jsonError ? (
                <div className="app-node-sidebar__json-error">{jsonError}</div>
              ) : null}
              {!jsonError && jsonStatus ? (
                <div className="app-node-sidebar__json-status">
                  {jsonStatus}
                </div>
              ) : null}
            </>
      ...
      >
        <div className="tabs-toolbar">
          {tabs.map((tab) => (
            <div key={tab.id} className="app-node-sidebar__json-actions">
              <button
                type="button"
                className={
                  tab.id === activeTabId ? "import-open-workflow-btn" : ""
                }
          ...
          <button type="button" onClick={onCreateBlankTab}>
            +
          </button>
          <select
            value={projectToOpenId}
            onChange={(event) => setProjectToOpenId(event.target.value)}
          >
            <option value="">Load imported project...</option>
          ...
          <button
            type="button"
            disabled={!projectToOpenId}
            onClick={() => {
              onCreateTabFromImportedProject(projectToOpenId);
              setProjectToOpenId("");
            }}
          >
            Open
        ...
        {pendingRelationConnection ? (
          ...
          >
            <div className="app-side-panel__header">
              ...
              {relationPanelSourceNode?.data?.tableName ??
                pendingRelationConnection.sourceTableId}
            </div>
            <form onSubmit={onSaveRelation} className="app-side-panel__form">
              ...
              <div className="app-side-panel__actions">
                ...
                <button
                  type="submit"
                  className="app-side-panel__btn app-side-panel__btn--primary"
                >
